...
학생용 사례는 학습 목표를 바탕으로 별도로 설계한다. 기관명만 지우거나 원문을 번역하는 방식으로 만들지 않는다.

- 업무 설명·표 구성·행의 의미·필드명·값의 표현을 교육용으로 설계한다. 추가 예제의 이름·식별자·숫자·날짜·운영 규칙은 모두 새로 만든 가상 값으로 표시한다.
- 외부 프로젝트의 원문, 데이터, 합성 샘플, 데이터 사전, 양식, 코드, 화면을 복사·변환해 배포하지 않는다. 원본과 교육용 필드의 대응표도 공개 파일·테스트·커밋 메시지에 넣지 않는다.
- 배포 전 학생 문서와 생성 ZIP을 모두 검토한다. 교수자 검토 자료는 Git과 사이트 배포 대상에서 제외하며, 명시된 학생 파일만 ZIP에 포함한다.

현재 구매 기록은 교육용 필드 6개로 정의한다. 날짜와 시각을 분리하고, 영화·극장·계정을 가상 별칭으로 표현한다. 사용자가 지정한 문제 분류 원칙과 평가 목적은 유지한다.
