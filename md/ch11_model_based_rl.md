...
handoff: learned models for planning and policy learning; next, train using only a fixed log
questions:
  - Why a model?
  - Plan with it?
  - Learn from rollouts?
...
|---|---|
| Lectures 8 and 10 learned values and policies from experience. | Now learn dynamics and use predicted transitions to help control. |

Model error can affect both plans and learned policies. Follow the core model-use loop before differentiating an optimizer.

::: keypoint
Re-plan from a measured temperature and explain how each learner uses imagined data.
:::

### Learning route — use a learned model, then check what it changes
**Bring:** Bellman updates, feedback, SAC and the idea of a latent state.

| First pass | What to do |
|---|---|
| **Follow the idea** | Fit a model → MPC/PETS → Dyna/MBPO → Dreamer |
| **Work without the solution** | Re-plan from a measured temperature and explain how each learner uses imagined data. |
| **Return later** | KKT-based differentiable MPC, PILCO and GPS/PLATO are after the main route. |
...
### The price, named — model errors can change the plan
::: lede
...
::: cols
::: col What goes wrong
A model is fitted where the base policy went. Planning then evaluates it where the *plan* goes, which is somewhere else — and a planner can prefer actions whose predicted outcomes are overly optimistic.

$$p_{\pi_f}(s_t)\;\neq\;p_{\pi_0}(s_t)$$
:::
::: col.accent Where you have met this before
Recall Lecture 5: optimization can select optimistic model errors. A planner can encounter a similar risk when searching sequences of actions.
...
::: reveal
::: small
So the optimum found in a learned model is only ==optimum from the model's point of view==. The planning examples show ways to reduce or manage that risk.
:::
:::
...
::: quiz Why might a learned model reduce the number of real interactions needed for learning?
- =It can support planning and synthetic updates that reuse the information in real experience
- It guarantees that the final policy is better than every model-free policy
- It removes the need for an informative state representation
- Synthetic transitions are new measurements of the real environment
Imagined experience can be cheap to generate, but it inherits model error. Sample efficiency is a possible benefit to test under a fixed real-data budget, not a guarantee. Training cost, model accuracy and the task all matter.
:::
...
::: small
These are alternative structures, not a strict ranking of expressivity or planning difficulty. The advanced differentiable-control section studies models whose structure also helps optimization.
:::
...
### Temperature thread — re-plan after a model error
{sub: shared teaching example · predict before revealing the calculation}

The planning model is $x'=x+u$, with cost $(x')^2+u^2$. Starting at x = −2, it chooses u = 1 and predicts x′ = −1. The real next measurement is −1.5.

**Predict:** If the model has not yet been refitted, should the next plan start from −1 or from −1.5?

::: reveal
**Calculate and check.** Start from the measurement **−1.5**. With the current model, one-step replanning gives $u=-x/2=\mathbf{0.75}$; the stale open-loop continuation was 0.5.
:::

::: keypoint
Replanning corrects the starting state. Learning the model corrects the predicted response; those are different updates.
:::

### Try it — the response model is refitted
{sub: work independently · reveal only after writing an answer}

At the measured state x = −1.5, change the learned dynamics to $x'=x+gu$ with g = 0.5. For cost $(x+gu)^2+u^2$, solve $g(x+gu)+u=0$ for u.

::: reveal
**Check your answer.** $u^*=-gx/(1+g^2)=-0.5(-1.5)/1.25=\mathbf{0.6}$. Under this updated model the cost is $(-1.2)^2+0.6^2=\mathbf{1.8}$.
:::

::: keypoint
A fresh state estimate and a changed dynamics model can both change the action. MBPO and Dreamer also depend on the model used to generate predictions.
:::
...
::: quiz Why can long rollouts from a learned model become unreliable?
...
A model accurate to within $\epsilon$ per step is not accurate to within $\epsilon$ over fifty steps — the errors feed forward and the imagined state leaves the region the model was ever fitted on. Short rollouts branched from **real** states keep the model working where it is trustworthy, This shares a concern about model trust with Lecture 5 and Lecture 10, but uses a different mathematical intervention.
:::

## Act 3 — learn from short simulated experience
{short: ACT 3, num: Act 3}

**Q3.** How can a learned model supply useful training transitions?
...
::: quiz Two learned models have the same average one-step prediction error. Why can their policies perform differently?
- Equal average prediction error guarantees equal control quality
- Only training time can explain the difference
- =Their errors can occur in different states or quantities, and some errors change the action ranking more than others
- A stochastic model always produces a better policy
An aggregate prediction score hides where errors occur. A small error near a decision boundary may matter more than a larger error in an irrelevant observation. Evaluate the resulting decisions as well as prediction fit. Task-aware model training is one possible extension, developed in the appendix.
:::

## Act 4 — learn in latent imagination
{short: ACT 4, num: Act 4}

**Q4.** How can a world model train a policy without searching action sequences at every real step?
...
::: quiz The online model-based methods in this chapter can collect corrective experience. What does Lecture 12 remove during training?
- The possibility of fitting any model
- The reward values already recorded in the log
- Every assumption about state representation
- =The ability to collect new transitions: training must use a fixed log
A mistaken prediction about an unsupported action cannot be checked by trying it during offline training. This recalls Lecture 5's fixed-data design problem, now with sequential values and policies. Evaluation must distinguish what the log supports from what a model merely predicts.
:::
...
### What we hand on
| Model use learned here | How it helps choose an action |
|---|---|
| **MPC / PETS** | Predict candidate trajectories, execute one action, then re-plan from a measurement. |
| **Dyna / MBPO** | Generate short synthetic experience to improve a value function and policy. |
| **Dreamer** | Learn an actor and critic through imagined latent trajectories. |

These online methods use real experience to correct their learned models. Differentiating a planner for task learning or joint design is an **advanced extension** in the appendix.

::: keypoint
Lecture 12 asks what changes when training must use a fixed log. We will examine policy restrictions, value learning and the evidence needed to evaluate a new policy.
:::
...
A model can be given, unavailable, or learned. Explain whether the action comes from a fresh plan or a trained actor, and which real observations support its predictions. Next, remove new interaction during training.
...
::: small
**Why the dual step is cheap.** With $x^*(\lambda)=\argmin_x \mathcal L(x,\lambda)$ and $g(\lambda)=\mathcal L(x^*(\lambda),\lambda)$, the chain rule gives $dg/d\lambda = (d\mathcal L/dx^*)(dx^*/d\lambda) + d\mathcal L/d\lambda$, and the first term vanishes because $d\mathcal L/dx^*=0$ at the argmin. So $dg/d\lambda = d\mathcal L/d\lambda$ evaluated at $x^*$ — no derivative through the inner solve is needed. This envelope calculation differs from the full implicit derivative of an optimizer, developed in the advanced differentiable-control section.
...
| task loss instead of prediction loss | emphasize errors that affect decisions | advanced differentiable control |
...
::: reveal
Step 1 is Lecture 9. Step 2 is plain supervised learning. Step 3 is dual gradient ascent — and its derivation uses an envelope argument: at the inner optimum $d\mathcal L/dx^*=0$, so ==the gradient through the $\arg\min$ collapses to a single term.==
:::
...
## Advanced bridge — differentiating a model-based controller
{short: EXTENSION}
...
Every version in Act 2 fits $f_\theta$ by minimising $L\big(x_{t+1}, f_\theta(x_t,u_t)\big)$ — a loss that is ==task-independent and data-dependent==. An aggregate prediction objective need not emphasize the errors that most affect action selection. Its weights and the training distribution determine which errors it prioritizes.
...
## Further model uses — direct gradients, teachers and design
{short: EXTENSION}
...
::: reveal
Turbine layout with the yaw controller that will run on it; furnace geometry with the recipe that will heat it. The advanced differentiable-control section supplies a possible derivative: ==differentiate the optimal control through to the design, and optimise both together== rather than designing first and controlling afterwards.
:::
